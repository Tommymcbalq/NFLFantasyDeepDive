...
Outputs: results/market_prior.csv, results/tier_variances.csv
"""
import re
import numpy as np
import pandas as pd
...
YEARS = range(2015, 2025)   # 2025 ADP unavailable at source
...
tv.to_csv(f"{ROOT}/results/tier_variances.csv", index=False)
print("\ntau^2 by tier:")
...
panel.to_csv(f"{ROOT}/results/market_prior.csv", index=False)
...
knots.to_csv(f"{ROOT}/results/market_prior_iso_knots.csv", index=False)
