...
Outputs: results/market_prior_rb.csv, results/tier_variances_rb.csv,
         results/market_prior_iso_knots_rb.csv, results/sectionJ_pi_sigma_rb.csv
"""
import re
...
tv.to_csv(ROOT / "results/tier_variances_rb.csv", index=False)
print("\ntau^2 by tier (RB):")
...
panel.to_csv(ROOT / "results/market_prior_rb.csv", index=False)
knots = pd.DataFrame({"log_adp": iso.X_thresholds_, "m": iso.y_thresholds_})
knots.to_csv(ROOT / "results/market_prior_iso_knots_rb.csv", index=False)
...
pi["position"] = "RB"
pi.to_csv(ROOT / "results/sectionJ_pi_sigma_rb.csv", index=False)
